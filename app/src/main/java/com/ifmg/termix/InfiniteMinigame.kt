...
class InfiniteMinigame : AppCompatActivity() {
    ...
    private fun createKeyBoardGrid() {
        keyboardGridController = KeyboardGridController(
            context = this,
            gridLayout = infiniteMinigameBinding.keyboardGridInfinite,
    ...
    private fun submitWord() {
        ...
        if (letterGrid.currentRow < 6) {
            ...
            if (isCorrect || letterGrid.currentRow > 6) { // TODO quando acerta na última ainda dá pra selecionar o campo
                infiniteMinigameBinding.answerTxtInfinite.text = "Acertou, parabéns!"
                ...
                infiniteMinigameBinding.retryGameBtn.setVisibility(View.VISIBLE)
                infiniteMinigameBinding.retryGameBtn.setOnClickListener {
                    resetGameUI()
                }
            } else if (letterGrid.currentRow == 6) {
                infiniteMinigameBinding.answerTxtInfinite.text = "A resposta certa era: $correctWord"
                keyboardGridController.setEnterButtonEnabled(false)
    ...
    private fun getWordGame(): String{
        val infiniteWord = gameController.getRandomWord()

        // Não foi possível escolher uma palavra para o jogo
        if(!gameController.validateWord(infiniteWord)){
    ...
    private fun resetGameUI() {
        ...
        infiniteMinigameBinding.retryGameBtn.setVisibility(View.INVISIBLE)
        ...
        infiniteMinigameBinding.answerTxtInfinite.text = ""
    }
